...
# EMWIN Data Upload Script for MinIO
# This script syncs EMWIN data to MinIO and cleans up local files
...
BUCKET_NAME=${EMWIN_BUCKET_NAME:-goes-19-emwin}
SOURCE_DIR="/emwin"
LOG_PREFIX="[$(date '+%Y-%m-%d %H:%M:%S')]"

echo "$LOG_PREFIX Starting EMWIN upload process..."
...
echo "$LOG_PREFIX Found $file_count EMWIN files to process"
...
# Sync files to MinIO with public ACL (excluding m1 directory)
echo "$LOG_PREFIX Syncing EMWIN files to MinIO bucket: $BUCKET_NAME"
mc mirror --remove --exclude "m1/*" --exclude "*.tmp" --exclude "*.partial" "$SOURCE_DIR/" "minio/$BUCKET_NAME/"
...
echo "$LOG_PREFIX EMWIN upload completed successfully"
