...
while IFS= read -r -d '' FILE_PATH; do
    ...
    if [[ "$FILENAME" =~ _([0-9]{14})_ ]]; then
        ...
        echo "$LOG_PREFIX   Extracted date: $TARGET_DATE from timestamp: $TIMESTAMP"
        ...
        if [[ "$YEAR" -ge 2024 ]] && [[ "$YEAR" -le 2030 ]]; then
            ...
            echo "$LOG_PREFIX   Moving to: $TARGET_PATH"
            
            if mv "$FILE_PATH" "$TARGET_PATH"; then
                ((MOVED++))
                echo "$LOG_PREFIX   Success! ($MOVED/$TOTAL_FILES)"
            else
                echo "$LOG_PREFIX   Failed to move: $FILENAME (error: $?)"
                ((FAILED++))
            fi
        else
